...
import { projectTasksAPI, type ProjectTask } from '@/lib/project-tasks-api'
...
export function ProjectTaskList({ quoteId }: ProjectTaskListProps) {
  ...
  const loadTasks = async () => {
    try {
      setLoading(true)
      const response = await projectTasksAPI.getQuoteTasks(quoteId)
      if (response.success) {
  ...
  const handleToggleTask = async (taskId: number) => {
    try {
      const response = await projectTasksAPI.toggleTask(quoteId, taskId)
      if (response.success) {
  ...
  const handleAddTask = async () => {
    ...
    try {
      setSubmitting(true)
      const response = await projectTasksAPI.createTask(quoteId, {
        title: newTaskTitle,
  ...
  const handleDeleteTask = async (taskId: number) => {
    ...
    try {
      await projectTasksAPI.deleteTask(quoteId, taskId)
