...
export function useThemeManager() {
  ...
  const resetTheme = React.useCallback(() => {
    console.log('[useThemeManager] resetTheme called - removing all theme CSS variables')
  ...
  const applyTheme = React.useCallback((themeValue: string, darkMode: boolean) => {
    console.log('[useThemeManager] applyTheme called:', { themeValue, darkMode })
    const theme = colorThemes.find(t => t.value === themeValue)
    if (!theme) {
      console.warn('[useThemeManager] Theme not found:', themeValue)
      return
    }
    ...
    console.log('[useThemeManager] Applying CSS variables:', Object.keys(styles).length, 'variables')
  ...
  const applyTweakcnTheme = React.useCallback((themePreset: ThemePreset, darkMode: boolean) => {
    console.log('[useThemeManager] applyTweakcnTheme called:', { darkMode })
    // Reset and apply theme variables
    resetTheme()
  ...
  const applyImportedTheme = React.useCallback((themeData: ImportedTheme, darkMode: boolean) => {
    console.log('[useThemeManager] applyImportedTheme called:', { darkMode })
    const root = document.documentElement
    const themeVars = darkMode ? themeData.dark : themeData.light

    console.log('[useThemeManager] Applying CSS variables:', Object.keys(themeVars).length, 'variables')
  ...
  const applyRadius = (radius: string) => {
    console.log('[useThemeManager] applyRadius called:', radius)
